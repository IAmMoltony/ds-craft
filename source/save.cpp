               ...
               Player &player, s16 &currentLocation)
{
    ...
    mtnlogMessageTagC(MTNLOG_INFO, "save", "Loading world with name `%s' folder `%s'", name.c_str(), worldFolder.c_str());
    ...
    if (!fsDirExists(worldFolder.c_str()))
    {
        mtnlogMessageTagC(MTNLOG_ERROR, "save", "folder %s does not exist", worldFolder.c_str());
        return;
    }
    ...
    mtnlogMessageTagC(MTNLOG_INFO, "save", "current location is %d", currentLocation);
    ...
    while (std::getline(wld, line)) // for each line in the file
    {
        ...
        else if (split[0] == "sg") // sg <x> <y> <color>
        {
            ...
            {
                mtnlogMessageTagC(MTNLOG_WARNING, "save", "Unknown stained glass color str '%s', using orange for no reason", sgcs.c_str());
                color = StainedGlassColor::Orange;
            }
    ...
    while (std::getline(ss, line))
    {
        ...
        if (chestFile.bad())
        {
            mtnlogMessageTagC(MTNLOG_ERROR, "save", "bad chest file %s", line.c_str());
            continue;
        }

        mtnlogMessageTagC(MTNLOG_INFO, "save", "Loading chest with ID %d", chestID);
...
void renameWorld(const std::string &oldName, const std::string &newName)
{
    ...
    if (worldnamePos != std::string::npos)
    {
        ...
        // 10 is the length of "worldname " (space included)
