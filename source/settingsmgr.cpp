...
void SettingsManager::loadLanguageLegacy(void)
{
    ...
    {
        mtnlogMessageTagC(MTNLOG_INFO, "settingsmgr", "Loading legacy language setting");
        ...
        case '1':
            ...
            break;
        default:
            mtnlogMessageTagC(MTNLOG_WARNING, "settingsmgr", "Invalid language code '%c'; defaulting to English");
            Game::instance->lang = Language::English;
            break;
        }
    }
...
void SettingsManager::loadSettingsLegacy(void)
{
    ...
    mtnlogMessageTagC(MTNLOG_INFO, "settingsmgr", "Loading legacy settings");
    ...
    if (fsFileExists(std::string(std::string(mtnconfigGet("configDir")) + "/autosave.cfg").c_str()))
    {
        ...
        {
            mtnlogMessageTagC(MTNLOG_INFO, "settingsmgr", "Auto save every 1 second detected, changing to 15");
            autoSaveSeconds = 15;
        }
    }
    else
...
{
    mtnlogMessageTagC(MTNLOG_INFO, "settingsmgr", "Updating settings format");
...
{
    mtnlogMessageTagC(MTNLOG_INFO, "settingsmgr", "Saving settings");
    ...
    if (!settingsFile)
    {
        mtnlogMessageTagC(MTNLOG_ERROR, "settingsmgr", "error opening settings file: %s", strerror(errno));
        return;
    }
...
{
    mtnlogMessageTagC(MTNLOG_INFO, "settingsmgr", "Loading settings");
    ...
    while (std::getline(file, line))
    {
        ...
        else if (split[0] == "lang")
        {
            ...
            if (split[1] == "0")
                ...
                mtnlogMessageTagC(MTNLOG_WARNING, "settingsmgr", "Invalid language code %s", split[1].c_str());
        }
        ...
        {
            // invalid key
            mtnlogMessageTagC(MTNLOG_WARNING, "settingsmgr", "Invalid setting '%s', ignoring", split[0].c_str());
        }
    }
}
